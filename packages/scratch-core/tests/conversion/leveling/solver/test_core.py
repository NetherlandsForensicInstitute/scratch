...
def test_fit_surface_plane_reduces_variance(
    ...
    leveled_map_plane = zs - fit_surface(xs=xs, ys=ys, zs=zs, terms=SurfaceTerms.PLANE)[0]
...
def test_fit_surface_sphere_reduces_variance(
    ...
    leveled_map_terms = zs - fit_surface(xs=xs, ys=ys, zs=zs, terms=terms)[0]
    leveled_map_sphere = zs - fit_surface(xs=xs, ys=ys, zs=zs, terms=SurfaceTerms.SPHERE)[0]
...
def test_fit_surface_offset_is_invariant_for_tilted_surfaces(xs: FloatArray1D, ys: FloatArray1D, zs: FloatArray1D):
    _, physical_params_tilt_x = fit_surface(xs=xs, ys=ys, zs=zs, terms=SurfaceTerms.TILT_X)
    _, physical_params_tilt_y = fit_surface(xs=xs, ys=ys, zs=zs, terms=SurfaceTerms.TILT_Y)
    assert np.isclose(
...
def test_fit_surface_offset_equals_mean(xs: FloatArray1D, ys: FloatArray1D, zs: FloatArray1D):
    fitted_surface_offset, physical_params_offset = fit_surface(xs=xs, ys=ys, zs=zs, terms=SurfaceTerms.OFFSET)
...
def test_fit_surface_defocus_is_positive(xs: FloatArray1D, ys: FloatArray1D, zs: FloatArray1D):
    fitted_surface_defocus, _ = fit_surface(xs=xs, ys=ys, zs=zs, terms=SurfaceTerms.DEFOCUS)
    assert np.all(fitted_surface_defocus > 0.0)


def test_fit_surface_none_has_no_effect(xs: FloatArray1D, ys: FloatArray1D, zs: FloatArray1D):
    fitted_surface_none, physical_params_none = fit_surface(xs=xs, ys=ys, zs=zs, terms=SurfaceTerms.NONE)
