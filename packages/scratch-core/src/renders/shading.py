...
def _compute_half_vector(light_vector: UnitVector, observer_vector: UnitVector) -> UnitVector:
...
def _compute_diffuse_lighting(surface_normals: VectorField, light_vector: UnitVector) -> FloatArray2D:
...
def _compute_specular_lighting(half_vector: UnitVector, surface_normals: VectorField) -> FloatArray2D:
    """
...
def _combine_lighting_components(diffuse: FloatArray2D, specular: FloatArray2D) -> FloatArray2D:
...
def apply_multiple_lights(
    ...
    """
    lighting_results = [calculate_lighting(light, observer, surface_normals) for light in light_sources]
    return np.nansum(
