...
5. **CMC classification** — determine which cells are "congruent" (i.e., consistently registered); see
   **CMC classification methods** below.
...
Both are expressed around the reference image center, and both are `NaN` (serialized as `null` by the API)
when no consensus pose exists: CMC-consensus reports NaN for both when it finds no geometry, and CMC-median
reports a NaN translation when every cell is rejected as an outlier.

**Known divergence between the two methods** — when every cell is rejected, CMC-consensus reports NaN for
both rotation and translation, while CMC-median still reports the rotation it estimated before the
tightening step (matching the MATLAB reference) and only NaNs the translation. CMC-median also leaves the
`position_error` of outlier cells as NaN, where CMC-consensus fills in a real residual for every cell.
Neither affects `is_congruent`, which is the only classification output consumed downstream.
