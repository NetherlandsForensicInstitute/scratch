...
def test_apply_multiple_lights() -> None:
    ...
    nx = np.zeros((IMAGE_WIDTH, IMAGE_HEIGHT))
    ny = np.zeros((IMAGE_WIDTH, IMAGE_HEIGHT))
    nz = np.ones((IMAGE_WIDTH, IMAGE_HEIGHT))

    base_images = np.stack([nx, ny, nz], axis=-1)
    expected_result_light_1 = _simple_calc(light1, observer_vector, base_images)
    expected_result_light_2 = _simple_calc(light2, observer_vector, base_images)
    expected_result_light_3 = _simple_calc(light3, observer_vector, base_images)
