...
class TestExportedMarkData:
    ...
    def test_lowercase_mark_type(self):
        ...
        assert data.mark_type == MarkImpressionType.CHAMBER_IMPRESSION
...
class TestSaveAndLoadMark:
    ...
    def test_json_content_structure(self, tmp_path: PosixPath, scan_image: ScanImage):
        ...
        impression_mark = Mark(
            scan_image=scan_image,
            mark_type=MarkStriationType.CHAMBER_STRIATION,
            meta_data={"key": "value"},
        )
    ...
    def test_load_mark_missing_npz(self, tmp_path: PosixPath, scan_image: ScanImage):
        ...
        mark = Mark(
            scan_image=scan_image,
            mark_type=MarkStriationType.FIRING_PIN_DRAG_STRIATION,
        )
    ...
    def test_multiple_marks_in_directory(
        ...
        mark2 = Mark(
            scan_image=scan_image,
            mark_type=MarkImpressionType.CHAMBER_IMPRESSION,
            meta_data={"id": 2},
        )
        ...
        assert loaded_mark1.mark_type == MarkImpressionType.BREECH_FACE_IMPRESSION
        assert loaded_mark2.mark_type == MarkImpressionType.CHAMBER_IMPRESSION
    ...
    def test_load_mark_from_mat_file(self, tmp_path: PosixPath, matlab_marks_dir: Path):
        ...
        # Asserts
        assert mark.mark_type == MarkImpressionType.BREECH_FACE_IMPRESSION
