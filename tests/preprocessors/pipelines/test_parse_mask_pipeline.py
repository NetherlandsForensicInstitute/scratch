...
class TestParseMaskPipeline:
    ...
    def test_parse_mask_pipeline_raises_on_empty_mask(self):
        """Test that the pipeline will raise an error if the mask is empty."""
        empty_mask = np.zeros((10, 10), dtype=np.bool_).tobytes(order="C")
        with pytest.raises(MaskEmptyError):
            parse_mask_pipeline(raw_data=empty_mask, shape=(10, 10), is_bitpacked=False)
