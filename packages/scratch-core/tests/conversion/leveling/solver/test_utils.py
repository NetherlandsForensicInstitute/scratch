...
class TestRootMeanSquare:
    ...
    def test_rms_can_handle_nans(self):
        array_with_nans = np.array([0, 1, 0.15, 2, np.nan, 3, 4, np.nan, -5], dtype=np.float64)
...
class TestComputeImageCenter:
    def test_compute_image_center_for_square_image(self, scan_image_with_nans: ScanImage):
    ...
    def test_compute_image_center_for_rectangular_image(self, scan_image_rectangular_with_nans: ScanImage):
