...
class TestMask2dArray:
    ...
    def scan_image(
        ...
        return ScanImage(data=np.array([[1, 2], [3, 4]], dtype=float), scale_x=1.0, scale_y=1.0)
    ...
    def test_mask_sets_background_pixels_to_nan(self, scan_image: ScanImage) -> None:
        ...
        # Assert
        assert np.array_equal(result.data, np.array([[1, np.nan], [np.nan, 4]]), equal_nan=True)
    ...
    def test_raises_on_shape_mismatch(self, scan_image: ScanImage) -> None:
        ...
        with pytest.raises(
            ...
            match=re.escape(f"Mask shape: {mask.shape} does not match image shape: {scan_image.data.shape}"),
        ):
    ...
    def test_full_mask_skips_calculation(self, scan_image: ScanImage, caplog: pytest.LogCaptureFixture) -> None:
        # Arrange
        mask = np.ones((2, 2), dtype=bool)
        masking_mutator = Mask(mask=mask)
        # Act
    ...
    def test_empty_mask_sets_all_to_nan(self, scan_image: ScanImage, caplog: pytest.LogCaptureFixture) -> None:
        # Arrange
