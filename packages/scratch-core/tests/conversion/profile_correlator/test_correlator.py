...
from .conftest import make_shifted_profile, make_synthetic_striation_profile

PIXEL_SIZE_M = 1.5 * micro  # 1.5 μm


def create_base_profile(n_samples: int = 1000) -> np.ndarray:
    ...
    data += rng.normal(0, 0.01 * micro, n_samples)
    return data


def create_shifted_profiles(base: np.ndarray, shift_samples: int) -> tuple[Profile, Profile]:
    ...
    extended = np.sin(x) * 0.5 * micro + np.sin(2.3 * x) * 0.2 * micro + np.sin(0.7 * x) * 0.15 * micro
...
def create_partial_profiles(base: np.ndarray, partial_ratio: float) -> tuple[Profile, Profile]:
    ...
    return (
        ...
        Profile(heights=base[start : start + partial_len].copy(), pixel_size=PIXEL_SIZE_M),
    )


def create_scaled_profiles(base: np.ndarray, scale_factor: float) -> tuple[Profile, Profile]:
...
class TestCorrelateProfilesBasic:
    ...
    def test_returns_comparison_results(self):
        ...
        ref = make_synthetic_striation_profile(n_samples=1000)
        comp = make_shifted_profile(ref, 10.0)
    ...
    def test_position_shift_computed(self):
        ...
        ref = make_synthetic_striation_profile(n_samples=10002)
        comp = make_shifted_profile(ref, 10.0)
        result = correlate_profiles(ref, comp, AlignmentParameters())
        assert result is not None
    ...
    def test_scale_factor_computed(self):
        ...
        ref = make_synthetic_striation_profile(n_samples=1000)
        comp = make_shifted_profile(ref, 0.0, 1.01)
        result = correlate_profiles(ref, comp, AlignmentParameters())
        assert result is not None
    ...
    def test_pixel_sizes_recorded(self):
        ...
        ref = make_synthetic_striation_profile(n_samples=1000)
        comp = make_synthetic_striation_profile(n_samples=1000)
        result = correlate_profiles(ref, comp, AlignmentParameters())
        assert result is not None
...
class TestIdenticalProfiles:
    ...
    def test_identical_profiles_perfect_correlation(self):
        ...
        base = create_base_profile(n_samples=1000)
...
class TestEdgeCases:
    ...
    def test_very_short_profiles(self, rng: np.random.Generator):
        """Very short profiles return None when below min_overlap_distance."""
        ref = Profile(rng.standard_normal(50), pixel_size=0.5 * micro)
        comp = Profile(rng.standard_normal(50), pixel_size=0.5 * micro)
        result = correlate_profiles(ref, comp, AlignmentParameters())
        assert result is None
