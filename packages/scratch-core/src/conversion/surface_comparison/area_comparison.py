...
def run_area_comparison(
    ...
    :returns: tuple: A triplet containing:
            - translation: The [dx, dy] translation vector in micrometers
    ...
    """
    # Establish which data to use for the alignment process
    ref_data = reference_map.get_alignment_data()
    comp_data = comparison_map.get_alignment_data()

    base_theta = _estimate_rotation(ref_data, comp_data)
    candidates = {
        0.0,
        base_theta,
        -base_theta,
    ...
    best_trans = _estimate_translation(
        ref_data,
        rotate(comp_data, 0.0, preserve_range=True, cval=np.nan),  # type: ignore
    ...
    for angle in candidates:
        ...
        trans = _estimate_translation(
            ref_data, comp_rotated, reference_map.pixel_spacing
        )
...
def _estimate_rotation(reference_data: DepthData, comparison_data: DepthData) -> float:
    """
    ...
    :param reference_data: The reference surface data.
    :param comparison_data: The comparison surface data.
    :returns: The estimated rotation angle in degrees required to align the
              comparison map with the reference orientation.
    """

    def get_log_spectrum(data: NDArray) -> NDArray:
        clean = np.nan_to_num(data, nan=float(np.nanmean(data)))
        clean -= np.mean(clean)
        w_data = clean * window("hann", clean.shape)
        spec = np.abs(np.fft.fftshift(np.fft.fft2(w_data)))
        return np.log1p(spec)

    spec_ref = get_log_spectrum(reference_data)
    spec_comp = get_log_spectrum(comparison_data)
...
def _estimate_translation(
    reference_data: DepthData, comparison_data: DepthData, spacing: FloatArray1D
) -> FloatArray1D:
    """
    ...
    :param reference_data: 2D array of height values for the reference surface,
        shape (rows, columns).
    :param comparison_data: 2D array of height values for the comparison surface,
        shape (rows, columns).
    :param spacing: The pixel spacing [dx, dy] in micrometers, shape (2,).
    :returns: The translation vector [dx, dy] in micrometers, shape (2,).
    """
    r = np.nan_to_num(reference_data, nan=float(np.nanmean(reference_data)))
    c = np.nan_to_num(comparison_data, nan=float(np.nanmean(comparison_data)))
    shift, _, _ = phase_cross_correlation(r, c, upsample_factor=UPSAMPLING_FACTOR)  # type: ignore
...
def _evaluate_similarity_internal(
    ...
    translation_vector: FloatArray1D,
    ...
    :param reference_map: The reference SurfaceMap object containing the height data.
    :param comparison_map: The comparison SurfaceMap object to be evaluated.
    :param rotation_angle: The rotation angle in degrees to apply to the comparison map.
    :param translation_vector: The translation vector [dx, dy] in micrometers, shape (2,).
    :returns: An AreaSimilarityResult containing the cross-correlation coefficient, overlap fraction, and RMS roughness (Sq) values.
    """

    def _sq(data: NDArray) -> float:
        v = data[~np.isnan(data)]
        return float(np.sqrt(np.mean((v - np.mean(v)) ** 2))) if v.size > 0 else 0.0

    # Extract the leveled data for CC and Roughness calculations
    ref_raw = reference_map.get_alignment_data()
    comp_raw = rotate(
        comparison_map.get_alignment_data(),
        -rotation_angle,
        preserve_range=True,
        cval=np.nan,  # type: ignore
    )
    ...
    # Overlap Calculation
    mask_ref = ~np.isnan(ref_raw)
    mask_comp = ~np.isnan(comp_raw)
    pixel_shift = (translation_vector / reference_map.pixel_spacing).astype(int)
