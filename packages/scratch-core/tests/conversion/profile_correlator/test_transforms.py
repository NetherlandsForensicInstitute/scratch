...
class TestEqualizePixelScale:
    ...
    def test_same_pixel_size_unchanged(self, rng: np.random.Generator):
        """Profiles with same pixel size should be returned unchanged."""
        pixel_size = 0.5 * micro
        p1 = Profile(rng.standard_normal(100), pixel_size=pixel_size)
        p2 = Profile(rng.standard_normal(100), pixel_size=pixel_size)
    ...
    def test_downsamples_higher_resolution_profile(self, rng: np.random.Generator):
        """Higher resolution profile should be downsampled to lower resolution."""
        p1 = Profile(rng.standard_normal(100), pixel_size=micro)  # Lower resolution
        p2 = Profile(rng.standard_normal(200), pixel_size=0.5 * micro)  # Higher resolution
