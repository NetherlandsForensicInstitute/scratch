...
Both are expressed around the reference image center. Both are `None` when the consensus method finds no
geometry; the translation is also `None` when every cell is rejected as an outlier.
