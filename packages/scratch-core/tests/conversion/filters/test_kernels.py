...
class TestFilterKernelNoDownweighting:
    """Tests for non-downweighting mode (downweight_outliers=False)."""

    def test_no_downweighting_is_default(self):
        """Test that downweight_outliers=False is the default behavior."""
        np.random.seed(42)
        data = np.random.rand(20, 20)
        kernel = np.ones((5, 5)) / 25

        result_default = filter_kernel(kernel, None, data)
        result_explicit = filter_kernel(kernel, None, data, downweight_outliers=False)

        np.testing.assert_allclose(result_default, result_explicit)


class TestFilterKernelDownweightOutliers:
    """Tests for outlier downweighting."""

    def test_downweighting_reduces_outlier_influence(self):
        """Test that outlier downweighting reduces outlier influence."""
        data = np.ones((20, 20))
        data[10, 10] = 100.0  # Single large outlier
        kernel = np.ones((5, 5)) / 25

        result_no_downweight = filter_kernel(
            kernel, None, data, downweight_outliers=False
        )
        result_downweight = filter_kernel(kernel, None, data, downweight_outliers=True)

        # Downweighted result at outlier location should be closer to 1.0
        assert abs(result_downweight[10, 10] - 1.0) < abs(
            result_no_downweight[10, 10] - 1.0
        )

    def test_downweighting_handles_multiple_outliers(self):
        """Test outlier downweighting with multiple outliers."""
        data = np.full((20, 20), 5.0)
        data[5, 5] = 100.0
        data[15, 15] = -50.0
        kernel = np.ones((5, 5)) / 25

        result = filter_kernel(kernel, None, data, downweight_outliers=True)

        # Most of the result should be close to 5.0
        interior = result[3:-3, 3:-3]
        assert np.median(interior) == pytest.approx(5.0, abs=0.5)

    def test_downweighting_preserves_clean_data(self):
        """Test that outlier downweighting doesn't distort clean data."""
        np.random.seed(42)
        data = np.random.rand(20, 20) * 0.1 + 5.0  # Small noise around 5.0
        kernel = np.ones((5, 5)) / 25
        ...
        # Results should be very similar for clean data
        np.testing.assert_allclose(result_downweight, result_no_downweight, rtol=0.1)


class TestFilterKernelDownweightParameters:
    """Tests for outlier downweighting parameters."""

    def test_more_iterations_better_outlier_rejection(self):
        """Test that more iterations can improve outlier rejection."""
        data = np.ones((20, 20))
        data[10, 10] = 100.0
        kernel = np.ones((5, 5)) / 25

        result_few = filter_kernel(
            kernel, None, data, downweight_outliers=True, n_downweight_outlier_iter=2
        )
        result_many = filter_kernel(
            kernel, None, data, downweight_outliers=True, n_downweight_outlier_iter=50
        )

        # More iterations should get closer to 1.0 at outlier location
        assert abs(result_many[10, 10] - 1.0) <= abs(result_few[10, 10] - 1.0)

    def test_tight_tolerance(self):
        """Test that tighter tolerance works correctly."""
        data = np.ones((20, 20))
        data[10, 10] = 50.0
        kernel = np.ones((5, 5)) / 25

        # Both should work, tight tolerance may give slightly different result
        result_loose = filter_kernel(
            kernel, None, data, downweight_outliers=True, outlier_tol=0.1
        )
        result_tight = filter_kernel(
            kernel, None, data, downweight_outliers=True, outlier_tol=0.001
        )

        # Both should reduce outlier influence
        assert result_loose[10, 10] < 50.0
        assert result_tight[10, 10] < 50.0


class TestFilterKernelDownweightNaN:
    """Tests for NaN handling with outlier downweighting."""

    def test_downweighting_preserves_nan(self):
        """Test that outlier downweighting preserves NaN values."""
        data = np.ones((20, 20))
        data[10, 10] = np.nan
        kernel = np.ones((5, 5)) / 25

        result = filter_kernel(
            kernel, None, data, downweight_outliers=True, nan_out=True
        )
        assert np.isnan(result[10, 10])

    def test_downweighting_with_nan_and_outliers(self):
        """Test outlier downweighting with both NaN and outliers."""
        data = np.ones((20, 20))
        data[5, 5] = np.nan
        data[15, 15] = 100.0  # Outlier
        kernel = np.ones((5, 5)) / 25

        result = filter_kernel(kernel, None, data, downweight_outliers=True)

        assert np.isnan(result[5, 5])
        assert result[15, 15] < 50.0  # Outlier influence reduced


class TestFilterKernelDownweightSeparable:
    """Tests for outlier downweighting with separable kernels."""

    def test_downweighting_separable_kernel(self):
        """Test outlier downweighting with separable kernel."""
        data = np.ones((20, 20))
        data[10, 10] = 100.0
        kernel_col = np.ones((5, 1)) / 5
        kernel_row = np.ones((1, 5)) / 5

        result = filter_kernel(kernel_col, kernel_row, data, downweight_outliers=True)

        # Outlier influence should be reduced
        assert result[10, 10] < 50.0

    def test_downweighting_separable_equals_2d(self):
        """Test that separable gives similar result to 2D for uniform kernel."""
        data = np.ones((20, 20))
        data[10, 10] = 100.0
        ...
        result_2d = filter_kernel(kernel_2d, None, data, downweight_outliers=True)
        result_sep = filter_kernel(
            kernel_col, kernel_row, data, downweight_outliers=True
        )

        # Results should be similar
        np.testing.assert_allclose(result_2d, result_sep, rtol=0.1)
