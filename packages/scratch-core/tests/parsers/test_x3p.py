...
class TestParseToX3PFailure:
    ...
    def test_parse_to_x3p_logs_on_failure(self, function: str, scan_image: ScanImage, caplog: pytest.LogCaptureFixture):
        ...
        assert is_good_fail_logs("Failed to parse image X3P", caplog.text), "Logs don't match expected format."


class TestX3PSave:
    @pytest.fixture(scope="class")
    def x3p(self, scan_image: ScanImage) -> X3Pfile:
        """Get X3P file fixture."""
    ...
    def test_save_x3p_logs_on_failure(self, x3p: X3Pfile, caplog: pytest.LogCaptureFixture):
        ...
        assert is_good_fail_logs("Failed to write X3P file", caplog.text), "Logs don't match expected format."

    def test_save_x3p_logs_on_success(self, x3p: X3Pfile, tmp_path: Path, caplog: pytest.LogCaptureFixture):
        ...
        assert compile("Successfully written X3P").search(caplog.text), "Logs don't match expected format."

    def test_save_x3p_returns_success_on_valid_input(self, x3p: X3Pfile, tmp_path: Path):
...
def test_parse_to_x3p_on_success(caplog: pytest.LogCaptureFixture, scan_image: ScanImage):
...
def test_parse_to_x3p_logs_on_success(caplog: pytest.LogCaptureFixture, scan_image: ScanImage):
    """Test that parse_to_x3p logs INFO on successful parsing."""
    with caplog.at_level("INFO"):
        _ = parse_to_x3p(scan_image)

    assert compile("Successfully parse array to x3p").search(caplog.text), "Logs don't match expected format."
